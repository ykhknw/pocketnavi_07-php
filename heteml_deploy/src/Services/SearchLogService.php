...
class SearchLogService {
    ...
    private function generateLinkFromSearchData($searchData) {
        ...
        // デバッグ用ログ
        error_log("Generate link - pageType: " . ($pageType ?? 'NULL') . ", identifier: " . ($identifier ?? 'NULL') . ", searchType: " . ($searchType ?? 'NULL') . ", query: " . ($query ?? 'NULL'));
        error_log("Full searchData: " . json_encode($searchData));
        ...
        switch ($searchType) {
            case 'architect':
                ...
                error_log("Architect filters: " . json_encode($filters));
                
                // ページ閲覧ログの場合（pageTypeが設定されている）
                if (isset($filters['pageType']) && $filters['pageType'] === 'architect') {
                    $link = "/architects/{$filters['identifier']}/";
                    error_log("Architect link from page view: " . $link);
                    return $link;
                }
                
                // 検索ログの場合
                if (isset($filters['architect_slug'])) {
                    $link = "/architects/{$filters['architect_slug']}/";
                    error_log("Architect link from search filters: " . $link);
                    return $link;
                }
                ...
                if ($architectData && isset($architectData['slug'])) {
                    $link = "/architects/{$architectData['slug']}/";
                    error_log("Architect link from direct query: " . $link);
                    return $link;
                }
                
                $link = "/index.php?q=" . urlencode($query) . "&type=architect";
                error_log("Architect fallback link: " . $link);
                return $link;
                
            case 'prefecture':
                // 都道府県検索の場合、filtersからprefecturesEnを取得
                $filters = json_decode($searchData['filters'] ?? '{}', true);
                error_log("Prefecture filters: " . json_encode($filters));
                
                // ページ閲覧ログの場合（pageTypeが設定されている）
                if (isset($filters['pageType']) && $filters['pageType'] === 'prefecture') {
                    $link = "/index.php?prefectures=" . urlencode($filters['identifier'] ?? $query);
                    error_log("Prefecture link from page view: " . $link);
                    return $link;
                }
                
                // 検索ログの場合
                if (isset($filters['prefecture_en'])) {
                    $link = "/index.php?prefectures=" . urlencode($filters['prefecture_en']);
                    error_log("Prefecture link from search filters: " . $link);
                    return $link;
                }
                ...
                error_log("Prefecture link from conversion: " . $link);
                return $link;
                
            case 'building':
                // 建築物検索の場合、filtersからslugを取得
                $filters = json_decode($searchData['filters'] ?? '{}', true);
                error_log("Building filters: " . json_encode($filters));
            ...
            default:
                $link = "/index.php?q=" . urlencode($query);
                error_log("Default link: " . $link);
                return $link;
        }
    }
    ...
    public function getPopularSearchesForSidebar($limit = 5) {
        ...
        try {
            ...
            // デバッグ用：SQLクエリの結果をログ出力
            error_log("SQL query result count: " . count($searches));
            foreach ($searches as $index => $search) {
                error_log("Search $index: " . json_encode($search));
            }
            
            // リンク情報を追加
            foreach ($searches as &$search) {
                // デバッグ用ログ（リンク生成前）
                error_log("Before link generation - query: " . $search['query'] . ", search_type: " . ($search['search_type'] ?? 'NULL') . ", page_type: " . ($search['page_type'] ?? 'NULL'));
                
                $search['link'] = $this->generateLinkFromSearchData($search);
                // countフィールドを追加（サイドバー表示用）
                $search['count'] = $search['total_searches'];
                // デバッグ用ログ
                error_log("Sidebar search data: " . json_encode($search));
                error_log("Generated link: " . $search['link']);
            }
