...
echo "<h1>実際の建築家ページのパラメータデバッグ</h1>";
...
echo "<h2>実際の建築家ページの処理</h2>";
if ($isArchitectPage) {
    $architectSlug = $_GET['architects_slug'];
    echo "<p>建築家スラッグ: " . htmlspecialchars($architectSlug) . "</p>";
    ...
    echo "<p>検索結果: " . $total . "件</p>";
    
    if (!empty($buildings)) {
        echo "<h3>最初の建築物のバッジURL生成テスト</h3>";
        $building = $buildings[0];
        
        echo "<h4>建築物: " . htmlspecialchars($building['title']) . "</h4>";
        
        // prefecturesバッジのURL生成テスト
        if (!empty($building['prefectures'])) {
            echo "<h5>prefecturesバッジのURL生成</h5>";
            ...
            echo "<p><strong>生成されたURL:</strong> " . htmlspecialchars($url) . "</p>";
            echo "<p><strong>prefectures:</strong> " . htmlspecialchars($building['prefectures']) . " / " . htmlspecialchars($building['prefecturesEn']) . "</p>";
        }
    }
} else {
    echo "<p>建築家ページではありません。</p>";
}
